...
Generic Types are a tool for effectively handling the duplication of concepts.

Generics: abstract stand-ins for concrete types or other properties. We can express the behavior of 
generics or how they relate to other generics without knowing what will be in their place when compiling and running 
the code.

Lifetimes: a variety of generics that give the compiler information about how references relate to each other. 
Lifetimes allow us to give the compiler enough information about borrowed values so that it can ensure references 
will be valid in more situations than it could without our help.

```rust
// To enable comparisons, the standard library has the std::cmp::PartialOrd trait that can be implemented on types
...
```rust
// this definition says that the Point<T> struct is generic over some type T, and the fields x and y are both that 
// same type, whatever that type may be.
struct Point<T> {
    x: T,
    y: T,
}
...
fn main() {
    ...
    let wont_work = Point { x: 5, y: 4.0 }; // Won't work
}
```
...
// The following code means the type Point<f32> will have a distance_from_origin method; other instances of Point<T> 
...
Generic type parameters in a struct definition aren’t always the same as those you use in that same struct method 
signatures. The following code uses the generic types `X1` and `Y1` for the `Point` struct and `X2` `Y2` for the mixup
method signature to make the example clearer. The method creates a new `Point` instance with the `x` value from the
`self` `Point` (of type `X1`) and the `y` value from the passed-in `Point` (of type `Y2`).
...
specify a small part of it. For example, we could define the `Summary` trait to have a `summarize_author` method whose 
implementation is required, and then define a `summarize` method that has a default implementation that calls the 
...
Note that it is **not** possible to call the default implementation from an overriding implementation of that same 
method.
...
_trait bound_; it looks like this:
...
Using `impl Trait` is appropriate if we want the following function to allow `item1` and `item2` to have different types 
(as long as both types implement `Summary`). If we want to force both parameters to have the same type, however, we
must use a trait bound, like the one on th2 second line:
...
Note that you can only use impl Trait if you’re returning a single type. For example, this code that returns either 
a `NewsArticle` or a `Tweet` with the return type specified as `impl Summary` wouldn’t work:
